function setRunMd5(callback) {
        ...
        if (fifterFile(_file, false)) {
            (function () {
                ...
                for (var j = 0; j < algorithms.length; j++) {
                    ...
                    if ($('[name="' + current.name + '-switch"]').is(':checked')) {
                ...
                progressiveRead(f, function (data, pos, file) {
                    // Work
                    ...
                    if (progress > lastprogress) {
                        ...
                        $('#' + uid).html(file.name + '（' + bytes2si2(pos, file.size, 2) + '）| 耗时: ' + digits(took, 2) + 's @ ' + bytes2si(pos / took, 2) + '/s | ' + new Date().getTime())
                    ...
                    if (doCRC32)
                        results += '<tr><td class="green strong algoname">CRC-32</td><td>' + decimalToHexString(crc32intermediate) + '</td></tr>';
                    ...
                    resultsValue = ((results).replace('<tr><td class="green strong algoname">MD5 Hash</td><td class="algoresult">', '').replace('</td></tr>', '')).toUpperCase();
                    var res = {
                        value: resultsValue,
                        name: f.name
                    };
    ...
    var hashFile = new Dropzone("#hash_file1");
